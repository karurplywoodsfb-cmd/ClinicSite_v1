...
// COMPLIANCE-UPDATED: DPDP Act 2023 consent integrated (Fixes D3)
...
const TAKEN_SLOTS = ["9:30 AM","11:00 AM","3:00 PM","5:00 PM"];
...
export default function BookingEngine({ clinic, services = [], hidePrice = false }) {
  ...
  const handleBook = async () => {
    ...
    if (!consentGiven)                  { setError("Please provide both data processing consents above to proceed."); return; }
      ...
      <div style={{ padding:"24px 28px" }}>
          ...
          <div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {(services.length > 0 ? services : [
                { name:"General Checkup & Cleaning", icon:"🔬", price:"₹500" },
                { name:"Dental Implants",            icon:"🔩", price:"₹25,000" },
                { name:"Root Canal Treatment",       icon:"🦷", price:"₹3,000" },
                { name:"Teeth Whitening",            icon:"✨", price:"₹3,500" },
                { name:"Braces & Aligners",          icon:"😁", price:"₹18,000" },
                { name:"Tooth Extraction",           icon:"💨", price:"₹500" },
                { name:"Kids Dentistry",             icon:"👶", price:"₹400" },
                { name:"Dentures & Crowns",          icon:"👑", price:"₹5,000" },
              ]).filter(s => s.is_active !== false).map((svc, i) => (
                <button key={i} onClick={() => { set("service", svc.name); setStep(2); }}
                  style={{
        ...
        {step === 2 && (
          <div>
              ...
              {dates.map((d, i) => {
                ...
                return (
                  <button key={i} onClick={() => !isSun && set("date", d)} disabled={isSun}
                    style={{ flexShrink:0, width:60, padding:"10px 0", borderRadius:10,
                      cursor: isSun?"not-allowed":"pointer",
                      background: sel?"#1565c0": isSun?"#f8fafc":"white",
                      border: `1.5px solid ${sel?"#1565c0":isSun?"#e8eef6":"#e8eef6"}`,
            ...
            {form.date && (
              <>
                <div style={{ fontSize:13, fontWeight:600, color:"#0b2545", marginBottom:10 }}>Available Slots</div>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8, marginBottom:20 }}>
                  {TIME_SLOTS.map((slot, i) => {
                    const taken = TAKEN_SLOTS.includes(slot);
                    const sel   = form.slot === slot;
                    return (
                      <button key={i} onClick={() => !taken && set("slot", slot)} disabled={taken}
                        style={{ padding:"8px 4px", borderRadius:8, fontSize:12,
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={() => setStep(1)} style={{ flex:1, padding:"12px", background:"white", border:"1.5px solid #dce8f5", borderRadius:10, color:"#64748b", fontSize:14, cursor:"pointer", fontFamily:"inherit" }}>← Back</button>
              <button onClick={() => form.date && form.slot && setStep(3)} disabled={!form.date || !form.slot}
                style={{ flex:2, padding:"12px", background:form.date&&form.slot?"#1565c0":"#dce8f5", border:"none", borderRadius:10, color:form.date&&form.slot?"white":"#94a3b8", fontSize:14, fontWeight:600, cursor:form.date&&form.slot?"pointer":"not-allowed", fontFamily:"inherit", transition:"all .2s" }}>
                Continue →
              </button>
            </div>
          </div>
        ...
        {step === 3 && (
          <div>
            ...
            {/* ── DPDP CONSENT BLOCK (Compliance Fix D3) ── */}
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={() => setStep(2)} style={{ flex:1, padding:"12px", background:"white", border:"1.5px solid #dce8f5", borderRadius:10, color:"#64748b", fontSize:14, cursor:"pointer", fontFamily:"inherit" }}>← Back</button>
              <button onClick={handleBook} disabled={loading || !consentGiven}
                style={{ flex:2, padding:"12px", border:"none", borderRadius:10, fontSize:14, fontWeight:600, fontFamily:"inherit", transition:"all .2s",
        ...
        {step === 4 && (
          <div style={{ textAlign:"center", padding:"12px 0" }}>
            ...
            <div style={{ fontSize:20, fontWeight:700, color:"#0b2545", marginBottom:8 }}>Appointment Confirmed!</div>
            ...
            <div style={{ background:"#f0f7ff", border:"1px solid #dce8f5", borderRadius:12, padding:"16px 20px", marginBottom:20, textAlign:"left" }}>
              {[["Service",form.service],["Date",form.date?.toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"long"})],["Time",form.slot],["Patient",form.name]].map(([k,v]) => (
            ...
            <div style={{ background:"rgba(34,197,94,0.05)", border:"1px solid #bbf7d0", borderRadius:8, padding:"10px 14px", marginBottom:18, fontSize:11, color:"#64748b", textAlign:"left" }}>
              🔒 Your consent has been recorded as required by the DPDP Act, 2023. You may withdraw consent at any time by contacting {clinic?.name}.
            </div>

            <a href={`https://wa.me/${clinic?.whatsapp?.replace(/\D/g,"")}`} target="_blank"
            ...
            <button onClick={() => { setStep(1); setForm({service:"",date:null,slot:"",name:"",phone:"",notes:""}); setBooked(null); setConsentGiven(false); }}
