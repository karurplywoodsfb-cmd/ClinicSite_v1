...
export default function TelehealthPlatform({ clinic, services = [], doctors = [], media = [], hours = [], branches = [], onBookClick }) {
              ...
              {activeServices.map((svc,i)=>(
                <Reveal key={svc.id||i} delay={i*0.05}>
                  <div style={{ background:C.card, borderRadius:12, padding:"24px 20px",
                    ...
                    {svc.price && !svc.hide_price && <div style={{ fontSize:12, fontWeight:600, color:C.teal, marginTop:10 }}>Fee: {svc.price}</div>}
                  </div>
                </Reveal>
              ))}
            </div>
